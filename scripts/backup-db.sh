...
DB_NAME="${DB_NAME:-piloto_db}"
DB_USER="${DB_USER:-piloto_user}"
BACKUP_DIR="${BACKUP_DIR:-/var/backups/piloto-db}"
RETENTION_DAYS="${RETENTION_DAYS:-30}"
TIMESTAMP=$(date '+%Y%m%d_%H%M%S')
BACKUP_FILE="$BACKUP_DIR/piloto_$TIMESTAMP.sql.gz"
...
find "$BACKUP_DIR" -name "piloto_*.sql.gz" -mtime +"$RETENTION_DAYS" -delete
BACKUP_COUNT=$(find "$BACKUP_DIR" -maxdepth 1 -name "piloto_*.sql.gz" 2>/dev/null | wc -l)
