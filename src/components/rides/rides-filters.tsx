...
import { Calendar, Car, ListFilter } from "lucide-react";
...
const SORT_OPTIONS = [
  ...
  { value: "gross_desc", label: "Maior valor" },
  { value: "gross_asc", label: "Menor valor" },
] as const;
...
export function RidesFilters({ className }: RidesFiltersProps) {
  ...
  return (
    ...
    >
      <div className="relative min-w-0 flex-1 sm:min-w-[160px]">
        ...
        <Label htmlFor="rides-filter-from" className="sr-only">
          Data inicial
        </Label>
        <Input
          id="rides-filter-from"
          type="date"
          value={from}
          onChange={(e) => {
            pushParams((next) => {
      ...
      <div className="relative min-w-0 flex-1 sm:min-w-[160px]">
        <Calendar className="pointer-events-none absolute left-3 top-1/2 size-4 -translate-y-1/2 text-muted-foreground" />
        <Label htmlFor="rides-filter-to" className="sr-only">
          Data final
        </Label>
        <Input
          id="rides-filter-to"
      ...
      <div className="relative min-w-0 flex-1 sm:min-w-[150px]">
        <Car className="pointer-events-none absolute left-3 top-1/2 z-10 size-4 -translate-y-1/2 text-muted-foreground" />
        <Select
          value={platform || "all"}
          onValueChange={(v) => {
            pushParams((next) => {
              if (v === "all") next.delete("platform");
              else next.set("platform", v);
              next.delete("page");
            });
          }}
        >
